...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($regType === 'salon') {
        ...
        if (empty($errors)) {
            ...
            $stmt = $conn->prepare("INSERT INTO salons (name, locality, address, tagline, price_min, price_max, rating, username, password, verified, rain_safe) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)");
            $stmt->bind_param("ssssiidssi", $name, $locality, $address, $tagline, $priceMin, $priceMax, $rating, $username, $hashedPassword, $rainSafe);

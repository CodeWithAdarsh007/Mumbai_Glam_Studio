...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($regType === 'salon') {
        ...
        if ($priceHaircut < 0 || $priceBridal < 0 || $priceMens < 0) {
        ...
        if (empty($errors)) {
            ...
            $stmt = $conn->prepare("INSERT INTO salons (name, locality, address, tagline, price_min, price_max, price_haircut, price_bridal, price_mens, rating, username, password, verified, rain_safe, description, services, facilities, contact_phone, contact_email, website, working_hours, established_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssiiiiidsssssssssi", $name, $locality, $address, $tagline, $priceMin, $priceMax, $priceHaircut, $priceBridal, $priceMens, $rating, $username, $hashedPassword, $rainSafe, $description, $services, $facilities, $contactPhone, $contactEmail, $website, $workingHours, $establishedYear);
        ...
        <div class="register-wrapper animate-in">
            ...
            <form id="register-form" method="POST" action="register.php" enctype="multipart/form-data" novalidate>
                ...
                <div id="salonForm" style="display: none;">
                    ...
                    <!-- Service Prices -->
                    <div class="form-group" style="background: var(--cream); padding: 16px; border-radius: var(--radius-md); border-left: 3px solid var(--teal); margin-top: 16px;">
                        <h4 style="margin: 0 0 12px 0; font-size: 1rem; color: var(--teal);">
                            <i class="fas fa-tags"></i> Service Prices
                        </h4>
                        <p style="font-size: 0.85rem; color: var(--charcoal-muted); margin-bottom: 12px;">
                            Set individual prices for each service you offer. Customers will see these when booking.
                        </p>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="price_haircut">Haircut & Styling (₹)</label>
                                <input type="number" name="price_haircut" id="price_haircut" class="form-control"
                                       placeholder="e.g., 500"
                                       min="0"
                                       value="<?php echo isset($_POST['price_haircut']) ? htmlspecialchars($_POST['price_haircut']) : ''; ?>">
                            </div>
                            <div class="form-group">
                                <label for="price_bridal">Bridal Package (₹)</label>
                                <input type="number" name="price_bridal" id="price_bridal" class="form-control"
                                       placeholder="e.g., 1500"
                                       min="0"
                                       value="<?php echo isset($_POST['price_bridal']) ? htmlspecialchars($_POST['price_bridal']) : ''; ?>">
                            </div>
                            <div class="form-group">
                                <label for="price_mens">Men's Grooming (₹)</label>
                                <input type="number" name="price_mens" id="price_mens" class="form-control"
                                       placeholder="e.g., 300"
                                       min="0"
                                       value="<?php echo isset($_POST['price_mens']) ? htmlspecialchars($_POST['price_mens']) : ''; ?>">
                            </div>
                        </div>
                        <small style="color:var(--charcoal-muted); font-size:0.75rem;">Set to 0 if you don't offer that service.</small>
                    </div>
